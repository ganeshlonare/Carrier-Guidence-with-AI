    ...
    <div className="p-10 bg-[#0a0a0a] min-h-screen flex flex-col items-center">
      ...
      {!loading && !error && roadmap.length > 0 && (
        <div className="relative w-full max-w-6xl">
          {/* Vertical line */}
          <div className="absolute left-1/2 top-0 h-full w-[1px] bg-white transform -translate-x-1/2"></div>
          ...
          {roadmap.map((step, index) => (
            ...
            >
              {/* Step content */}
              <div className="w-[45%] p-6 bg-slate-200 shadow-lg rounded-xl border border-gray-700 relative text-black">
                {/* Completion toggle button */}
                <button
                  onClick={() => toggleCompletion(index)}
                  className={`absolute w-12 h-12 flex items-center justify-center bg-indigo-500 text-white rounded-full shadow-md transition-transform hover:scale-110 ${
                ...
                {/* Connecting line */}
                <div
                  className={`absolute top-1/2 h-[1.3px] bg-white ${
                    index % 2 === 0 ? "-right-14" : "-left-14"
                  }`}
                  style={{
                    width: "calc(11%)", // Adjust based on card width and button size
                  }}
                ></div>
